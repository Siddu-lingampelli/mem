...
(function () {
  ...
  var saved = null;
  try {
    saved = localStorage.getItem("mem-docs-theme");
  } catch (e) {}
  ...
  themeBtn &&
    themeBtn.addEventListener("click", function () {
      ...
      try {
        localStorage.setItem("mem-docs-theme", cur === "light" ? "light" : "dark");
      } catch (e) {}
    });
  ...
  function refresh() {
    sections = (navLinks.length ? navLinks : tocLinks)
      .map(function (a) {
        return {
          id: a.getAttribute("data-target"),
          el: lookupSection(a.getAttribute("data-target")),
          link: a,
        };
      })
      .filter(function (s) {
  ...
  function openPalette() {
    ...
    pal.id = "palette";
    pal.style.cssText =
      "position:fixed;inset:0;z-index:100;display:flex;align-items:flex-start;justify-content:center;padding:12vh 20px 0;background:rgba(0,0,0,.45);backdrop-filter:blur(3px)";
    var box = document.createElement("div");
    box.style.cssText =
      "width:100%;max-width:520px;background:var(--bg);border:1px solid var(--border-strong);border-radius:12px;box-shadow:var(--shadow);overflow:hidden";
    var input = document.createElement("input");
    input.placeholder = "Jump to section…";
    input.style.cssText =
      "width:100%;border:0;outline:0;padding:16px 18px;font-size:15px;background:var(--bg);color:var(--text);font-family:var(--sans);border-bottom:1px solid var(--border)";
    ...
    function render(q) {
      ...
      var matches = ql
        ? index.filter(function (it) {
            return it.label.toLowerCase().indexOf(ql) >= 0 || it.id.indexOf(ql) >= 0;
          })
        : index;
      matches.slice(0, 8).forEach(function (it, i) {
        ...
        row.style.cssText =
          "padding:10px 18px;cursor:pointer;font-size:14px;border-bottom:1px solid var(--border)" +
          (i === 0 ? ";background:var(--accent-soft);color:var(--accent-text)" : "");
    ...
    function select(i) {
      ...
      if (items[i]) {
        items[i].style.background = "var(--accent-soft)";
        items[i].style.color = "var(--accent-text)";
    ...
    function go(it) {
      closePalette();
      var el = lookupSection(it.id);
      if (el) el.scrollIntoView({ _behavior: "smooth" });
    }
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        if (outEl && OUTPUTS[String(i + 1)]) {
          await new Promise(function (r) {
            setTimeout(r, parseInt(outEl.getAttribute("data-delay") || "300", 10));
          });
